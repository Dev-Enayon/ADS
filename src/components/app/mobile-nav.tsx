...
import { mobileNavItems, primaryNav, secondaryNav } from "@/components/app/nav";
...
export function MobileNav({ unread }: { unread: number }) {
  ...
  const moreActive = !mobileNavItems.some((item) =>
            ...
            <div className="grid gap-1">
              ...
              <div className="mt-2 flex items-center justify-between border-t border-border px-2 pt-3">
                <span className="text-xs font-medium uppercase tracking-wider text-muted-soft">
                  {unread > 0 ? `${unread} unread notifications` : "Account"}
                </span>
                <SignOutButton />
              </div>
            </div>
